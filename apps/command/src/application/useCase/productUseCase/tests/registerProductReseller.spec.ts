...
describe('RegisterResellerSaleUseCase', () => {
  ...
  let productDomainServiceMock: ProductDomainService<ISale>;

  beforeEach(() => {
    commandBusMock = {} as CommandBus;
    productDomainServiceMock = {} as ProductDomainService<ISale>;
    productUseCase = new RegisterResellerSaleUseCase(productDomainServiceMock, commandBusMock);
  });
  ...
  describe('registerResellerSale', () => {
    it('should register a reseller sale and call the productDomainService', () => {
      ...
      productDomainServiceMock.registerResellerSale = jest.fn(() => of(sale));

      // Act
      const result$ = productUseCase.registerResellerSale(product, branchId);
  ...
  describe('execute', () => {
    it('should call the registerResellerSale method with provided data', () => {
      ...
      productUseCase['registerResellerSale'] = jest.fn(() => of(sale));

      // Act
      const result$ = productUseCase.execute(product, branchId);

      // Assert
      expect(productUseCase['registerResellerSale']).toHaveBeenCalledWith(product, branchId);
      result$.subscribe((result) => {
        expect(result).toEqual(sale);
      });
    });
