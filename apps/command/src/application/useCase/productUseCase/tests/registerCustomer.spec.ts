...
describe('registerCustomerSaleUseCase', () => {
  ...
  beforeEach(() => {
    ...
    customerSaleUseCase = new registerCustomerSaleUseCase(commandBusMock, productDomainServiceMock);
  });
  ...
  describe('execute', () => {
    it('should call the registerCustomerSale method with provided products and branchId', () => {
      ...
      // Act
      customerSaleUseCase.execute(products, branchId);

      // Assert
      expect(customerSaleUseCase['registerCustomerSale']).toHaveBeenCalledWith(products, branchId);
    });
  });
});
